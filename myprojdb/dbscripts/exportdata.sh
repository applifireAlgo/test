...
echo $PATH
OSNAME=`uname -s`
ART_DATA_PATH=/tmp/applifire/db/Y5ZLTDXILQ4XBUGKBQTEYG/art/data
AST_DATA_PATH=/tmp/applifire/db/Y5ZLTDXILQ4XBUGKBQTEYG/ast/data
DB_NAME=applifire
USER=root
PASSWORD=root
PORT=3306
HOST=localhost
MYSQL=/usr/bin
PROJECT_ID=Y5ZLTDXILQ4XBUGKBQTEYG
...
$MYSQL/mysql -p$PORT -u$USER -p$PASSWORD $DB_NAME -e "SELECT   menu_id,   menu_tree_id,   menu_icon,   menu_action,   menu_commands,   display,   head_menu,   menu_label,    ui_type,   updated_by,   updated_date,   created_by,   created_date,   version_id,   active_status,   project_id,   project_version_id,   app_creator_id,   reff_id FROM aws_runtime_menu WHERE project_id='$PROJECT_ID' AND project_version_id='$PROJECT_VERSION_ID' INTO OUTFILE '$ART_DATA_PATH/art_menu.csv' FIELDS TERMINATED BY '#appfire#' LINES TERMINATED BY '\n'";

$MYSQL/mysql -p$PORT -u$USER -p$PASSWORD $DB_NAME -e "SELECT '18D01ABF-F632-496A-B379-FC50EDEAB8C0',menu_id,updated_by,updated_date,created_by,created_date,version_id,active_status,project_id,project_version_id,app_creator_id FROM aws_runtime_menu WHERE project_id='$PROJECT_ID' AND project_version_id='$PROJECT_VERSION_ID' INTO OUTFILE '$ART_DATA_PATH/art_user_menu.csv' FIELDS TERMINATED BY '#appfire#' LINES TERMINATED BY '\n'";
