...
$MYSQL/mysql --local-infile=1 -h$HOST -p$PORT -u$USER -p$PASSWORD $DB_NAME -e "load data local infile '$ART_DATA_PATH/art_menu.csv' into table art_menu FIELDS TERMINATED BY '#appfire#' LINES TERMINATED BY '\n' ";

$MYSQL/mysql --local-infile=1 -h$HOST -p$PORT -u$USER -p$PASSWORD $DB_NAME -e "load data local infile '$ART_DATA_PATH/art_user_menu.csv' into table art_user_menu FIELDS TERMINATED BY '#appfire#' LINES TERMINATED BY '\n' ";
