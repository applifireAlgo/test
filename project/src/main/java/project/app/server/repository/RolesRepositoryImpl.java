...
public class RolesRepositoryImpl extends SearchInterfaceImpl implements RolesRepository<Roles> {
    ...
    public List<Roles> findAll() throws SpartanPersistenceException {
        try {
            ...
            java.util.List<project.app.shared.authorization.Roles> query = emanager.createQuery("select u from Roles u where u.systemInfo.activeStatus=1").getResultList();
            return query;
    ...
    public List<Roles> save(List<Roles> entity) throws SpartanPersistenceException {
        try {
            ...
            for (int i = 0; i < entity.size(); i++) {
                project.app.shared.authorization.Roles obj = entity.get(i);
    ...
    public void delete(String id) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            project.app.shared.authorization.Roles s = emanager.find(project.app.shared.authorization.Roles.class, id);
            emanager.remove(s);
        } catch (javax.persistence.PersistenceException e) {
    ...
    public void deleteRoleMenuBridge(List<RoleMenuBridge> rolemenubridge) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            for (project.app.shared.authorization.RoleMenuBridge _rolemenubridge : rolemenubridge) {
                project.app.shared.authorization.RoleMenuBridge s = emanager.find(project.app.shared.authorization.RoleMenuBridge.class, _rolemenubridge.getRoleMenuMapId());
                emanager.remove(s);
            }
        } catch (javax.persistence.PersistenceException e) {
            throw new SpartanPersistenceException("Error in deleting entity", e);
    ...
    public Roles findById(String roleId) throws Exception, SpartanPersistenceException {
        try {
            ...
            project.app.shared.authorization.Roles listOfRoles = (project.app.shared.authorization.Roles) query.getSingleResult();
            return listOfRoles;
        } catch (javax.persistence.PersistenceException e) {
