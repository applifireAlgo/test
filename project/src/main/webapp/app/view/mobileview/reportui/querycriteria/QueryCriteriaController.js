Ext.define('Project.view.mobileview.reportui.querycriteria.QueryCriteriaController', {
	extend : 'Ext.app.ViewController',
	alias : 'controller.querycriteriaController',
	requires : [ 'Project.view.mobileview.fw.DateRange' ],
	reportView : null,
	reportViewController : null,
	datachart : null,
	init : function() {
		this.reportView = this.getView().up();
		this.reportViewController = this.getView().up().up().controller;
	...
	loadData : function(reportJSON) {
		...
		if (advanceConfigJson != undefined
				&& advanceConfigJson.hasOwnProperty("auto_refersh")
				&& advanceConfigJson["auto_refersh"] == "on") {
		...
		Ext.Array.each(reporUiObj.rptHrefQC, function(item, indx, items) {
			...
			if (widgets.length > 0) {
				...
				if (widgets[0].xtype == 'adatepickerfield') { // ||
					try {
						widgets[0]
								.setValue(Ext.Date.parse(item.value, 'Y-m-d'));
					} catch (e) {
					}
				} else {
	...
	refreshDepandantWidget : function(sender, newValue, oldValue, eOpts) {
		...
		Ext.Array.each(queryPanel.items.items, function(item, idx, items) {
			if (this.sender.widgetId == item.parentWidget) {
				var params = this.scope.getQueryCriteriaParams(this.queryPanel,
						item);
				var params = {};
	...
	renderComboboxStore : function(com, param) {
		try {
			var response = this.reportViewController.syncAjaxPOSTCall(
					this.reportViewController.restURL + com.serviceUrl, param);
			if (response == null) {
				return;
			}
	...
	loadWidgetsServices : function(widgets) {
		Ext.Array.each(widgets, function(widget, idx, items) {
			if (widget.serviceId != undefined && widget.serviceId.length > 0
	...
	setComboboxData : function(data, widget) {
		store = this.getStoreFromAPI(response[0],
				com.attribute != undefined ? com.attribute : "");
		com.bindStore(store);
	...
	loadDataInWidget : function(widget) {
		try {
			var response = this.reportViewController.syncAjaxGETCall(
					this.reportViewController.restURL + widget.serviceUrl,
					"jsonData={}");
			if (response == null) {
				return;
			}
	...
	clearData : function() {
		...
		Ext.Array.each(this.getView().items.items, function(item, ind, items) {
			try {
				item.reset();
				if (item.hasOwnProperty("parentWidget")
						&& item.parentWidget != 0) {
				...
				if (item.xtype != 'combobox') {
					if (item.xtype != 'combobox') {
						...
						if (item.xtype == 'datefield') {
							...
							var rec = item.down("#defaultsDate").getStore()
									.find("text", item.defaultValue);
							if (rec != -1) {
								item.down("#defaultsDate").setValue(
										item.down("#defaultsDate").getStore()
												.getAt(rec).data.name);
							}
						} else {
							item.setValue(value);
	...
	changeTimers : function(cmb) {
		...
		if (currentView.down("#isAutoRef")!=undefined && currentView.down("#isAutoRef").value==true) {
			this.reportViewController.updateTimer(this.reportViewController,
					cmb.getValue());
		}
