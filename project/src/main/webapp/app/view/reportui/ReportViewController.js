Ext.define('Project.view.reportui.ReportViewController', {
	...
	initObject : function() {
		this.queryCriteria = this.getView().down("#querycriteria");
	...
	loadDataPoints : function(datapoints, maindatapointpanel) {
		...
		for (var x = 0; x < datapoints.length; x++) {
			/*
			 * check if type of data point if the type is chart then set
			 * chatjson that used in data point widget to create chart point
			 * 
			 */
			if (datapoints[x].hasOwnProperty("type") && datapoints[x].type.toLowerCase() == 'chart') {
				...
				panelWiseJson[datapoints[x].displayName] = datapoints[x].value;
				datapointlist = datapointlist.concat(datapoints[x].value);
				/*
				 * datapointlist .push({ description : datapoints[x].label,
				 * statistic : datapoints[x].value, })
				 */
			}
		}

		/*
		 * calculate width of datapoint which will adjust in one row
		 */
		maindatapointpanel.removeAll(true);
		for ( var key in panelWiseJson) {

			// var defaulw = 100 - (panelWiseJson[key].length - 3);
			var width = 100 / panelWiseJson[key].length;

			width = width - 1;
			for (var x = 0; x < panelWiseJson[key].length; x++) {
				panelWiseJson[key][x]["width"] = width + "%";
			}

			if (panelWiseJson.hasOwnProperty(key)) {
				newDataPointPanel = {

					xtype : 'dataPointPanel',
					bodyStyle : 'background:#D8D8D8sssss',
					margin : '0 0 5 0',
					// itemId : "datapoint",
					title : key,
					// anchor : '100% 18%',
					...
					dpData : panelWiseJson[key]
				};
				maindatapointpanel.add(newDataPointPanel);

				/*
				 * var val = panelWiseJson[key]; console.log(val);
				 */
			}
			// maindatapointpanel.add(newDataPointPanel);
		}
		...
		Ext.Ajax.request({
			...
			success : function(response, currentObject, options) {
				...
				if (data != undefined && data.datapoints != undefined) {
					currentObject.scope.reportViewController.loadDataPoints(data.datapoints, currentObject.scope.view.query("mainDataPointPanel")[0]);
				}
			},
			failure : function() {
