package salesforce.app.server.service.sessionmgt;import javax.servlet.http.HttpServletRequest;
...
import com.spartan.shield.server.sessionService.SessionDataMgtService;
...
public void setSessionAttributeForObject(String key, Object value) {
	...
	public void removeSessionAttribute(String key) {
		...
		try {
			...
			//loginSession = loginSessionInterfaceRepository.findById(session.getAttribute("usidHash").toString());
	...
	public void removeSessionAllAttributes() {
		 ...
		 * if (loginSession != null) {
		 * 
		 * String sessionData = loginSession.getSessionData(); try { JSONObject
		 * sessionDataObject = new JSONObject(sessionData);
		 * sessionDataObject.remove(key); updateSession(loginSession,
		 * sessionDataObject.toString()); } catch (JSONException e) {
		 * e.printStackTrace(); }
		 */
	}
