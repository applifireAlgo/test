Ext.define('Salesforce.salesforce.web.salesforce.controller.authentication.PasswordAlgoMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.PasswordAlgoMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#PasswordAlgoTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#PasswordAlgo');
          var grid = this.view.down('#PasswordAlgoGrid');
          var tree = this.view.down('#PasswordAlgoTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#PasswordAlgoGrid');
               ...
               var formPanel = this.getView().up().down("#PasswordAlgo");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#PasswordAlgoTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#PasswordAlgo').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#PasswordAlgoGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.algoId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#PasswordAlgo').getViewModel().setData(responseData.response.data);
     },
