Ext.define('Salesforce.salesforce.web.salesforce.controller.contacts.CommunicationGroupMainController', {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CommunicationGroup');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var formPanel = this.getView().up().down("#CommunicationGroup");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CommunicationGroupTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CommunicationGroup').down('#' + me.column.dataIndex + '').store;
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.commGroupId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#CommunicationGroup').getViewModel().setData(responseData.response.data);
     },
