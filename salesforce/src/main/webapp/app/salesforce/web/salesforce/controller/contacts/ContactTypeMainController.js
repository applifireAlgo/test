Ext.define('Salesforce.salesforce.web.salesforce.controller.contacts.ContactTypeMainController', {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#ContactType');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var formPanel = this.getView().up().down("#ContactType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#ContactTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ContactType').down('#' + me.column.dataIndex + '').store;
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.contactTypeId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#ContactType').getViewModel().setData(responseData.response.data);
     },
