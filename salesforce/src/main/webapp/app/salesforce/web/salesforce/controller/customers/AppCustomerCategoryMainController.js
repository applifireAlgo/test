Ext.define('Salesforce.salesforce.web.salesforce.controller.customers.AppCustomerCategoryMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AppCustomerCategoryMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AppCustomerCategoryTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#AppCustomerCategory');
          var grid = this.view.down('#AppCustomerCategoryGrid');
          var tree = this.view.down('#AppCustomerCategoryTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#AppCustomerCategoryGrid');
               ...
               var formPanel = this.getView().up().down("#AppCustomerCategory");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#AppCustomerCategoryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AppCustomerCategory').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AppCustomerCategoryGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.appcustCategoryId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#AppCustomerCategory').getViewModel().setData(responseData.response.data);
     },
