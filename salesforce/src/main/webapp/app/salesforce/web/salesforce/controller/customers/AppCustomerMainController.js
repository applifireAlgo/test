Ext.define('Salesforce.salesforce.web.salesforce.controller.customers.AppCustomerMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AppCustomerMainController',
     ...
     init: function() {
          ...
          this.contactIdLoad();
          this.appCustomerTypeLoad();
          this.appCustomerCategoryLoad();
     },
     ...
     appCustomerTypeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/AppCustomerType/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#appCustomerType');
                    scope.sender.controller.assignAllComboData(scope.sender, 'appCustomerType', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     appCustomerCategoryLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/AppCustomerCategory/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#appCustomerCategory');
                    scope.sender.controller.assignAllComboData(scope.sender, 'appCustomerCategory', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    debugger;
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AppCustomerTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#AppCustomer');
          var grid = this.view.down('#AppCustomerGrid');
          var tree = this.view.down('#AppCustomerTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#AppCustomerGrid');
               ...
               var formPanel = this.getView().up().down("#AppCustomer");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#AppCustomerTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AppCustomer').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AppCustomerGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.appCustomerId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#AppCustomer').getViewModel().setData(responseData.response.data);
     },
