Ext.define('Salesforce.salesforce.web.salesforce.controller.customers.AppCustomerTypeMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AppCustomerTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AppCustomerTypeTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#AppCustomerType');
          var grid = this.view.down('#AppCustomerTypeGrid');
          var tree = this.view.down('#AppCustomerTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#AppCustomerTypeGrid');
               ...
               var formPanel = this.getView().up().down("#AppCustomerType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#AppCustomerTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AppCustomerType').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AppCustomerTypeGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.appcustTypeId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#AppCustomerType').getViewModel().setData(responseData.response.data);
     },
