Ext.define('Salesforce.salesforce.web.salesforce.controller.location.AddressTypeMainController', {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#AddressType');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var formPanel = this.getView().up().down("#AddressType");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#AddressTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AddressType').down('#' + me.column.dataIndex + '').store;
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.addressTypeId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#AddressType').getViewModel().setData(responseData.response.data);
     },
