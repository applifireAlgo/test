Ext.define('Salesforce.salesforce.web.salesforce.controller.location.CityMainController', {
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var formPanel = this.getView().up().down("#City");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CityTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#City').down('#' + me.column.dataIndex + '').store;
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.cityId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#City').getViewModel().setData(responseData.response.data);
     },
