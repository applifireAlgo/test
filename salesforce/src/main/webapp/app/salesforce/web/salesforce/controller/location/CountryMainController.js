Ext.define('Salesforce.salesforce.web.salesforce.controller.location.CountryMainController', {
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Country');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var formPanel = this.getView().up().down("#Country");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CountryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Country').down('#' + me.column.dataIndex + '').store;
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.countryId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Country').getViewModel().setData(responseData.response.data);
     },
