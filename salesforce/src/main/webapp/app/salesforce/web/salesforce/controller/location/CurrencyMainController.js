Ext.define('Salesforce.salesforce.web.salesforce.controller.location.CurrencyMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CurrencyMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CurrencyTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Currency');
          var grid = this.view.down('#CurrencyGrid');
          var tree = this.view.down('#CurrencyTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#CurrencyGrid');
               ...
               var formPanel = this.getView().up().down("#Currency");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CurrencyTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Currency').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CurrencyGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.currencyId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Currency').getViewModel().setData(responseData.response.data);
     },
