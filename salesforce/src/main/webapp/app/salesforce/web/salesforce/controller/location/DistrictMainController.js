Ext.define('Salesforce.salesforce.web.salesforce.controller.location.DistrictMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.DistrictMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#DistrictTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#District');
          var grid = this.view.down('#DistrictGrid');
          var tree = this.view.down('#DistrictTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#DistrictGrid');
               ...
               var formPanel = this.getView().up().down("#District");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#DistrictTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#District').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#DistrictGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.districtId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#District').getViewModel().setData(responseData.response.data);
     },
