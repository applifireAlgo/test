Ext.define('Salesforce.salesforce.web.salesforce.controller.location.RegionMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.RegionMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#RegionTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Region');
          var grid = this.view.down('#RegionGrid');
          var tree = this.view.down('#RegionTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#RegionGrid');
               ...
               var formPanel = this.getView().up().down("#Region");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#RegionTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Region').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#RegionGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.regionId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Region').getViewModel().setData(responseData.response.data);
     },
