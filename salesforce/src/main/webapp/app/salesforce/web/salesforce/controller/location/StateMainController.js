Ext.define('Salesforce.salesforce.web.salesforce.controller.location.StateMainController', {
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var formPanel = this.getView().up().down("#State");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#StateTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#State').down('#' + me.column.dataIndex + '').store;
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.stateId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#State').getViewModel().setData(responseData.response.data);
     },
