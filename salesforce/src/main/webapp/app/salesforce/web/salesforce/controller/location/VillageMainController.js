Ext.define('Salesforce.salesforce.web.salesforce.controller.location.VillageMainController', {
     extend: 'Buzzor.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.VillageMainController',
     ...
     onCountryIdChange: function(me, newValue, oldValue, eOpts) {
          ...
          this.onCountryIdChangeTalukaaId(me, newValue, oldValue, eOpts);
     },
     onStateIdChange: function(me, newValue, oldValue, eOpts) {
          ...
          this.onStateIdChangeTalukaaId(me, newValue, oldValue, eOpts);
     },
     onRegionIdChange: function(me, newValue, oldValue, eOpts) {
          this.onRegionIdChangeDistrictId(me, newValue, oldValue, eOpts);
          this.onRegionIdChangeTalukaaId(me, newValue, oldValue, eOpts);
     },
     onDistrictIdChange: function(me, newValue, oldValue, eOpts) {
          this.onDistrictIdChangeTalukaaId(me, newValue, oldValue, eOpts);
     },
     ...
     onCountryIdChangeTalukaaId: function(me, newValue, oldValue, eOpts) {
          debugger;
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/Taluka/findByCountryId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var talukaaIdCombo = scope.element.up('form').down('#talukaaId');
                    scope.sender.controller.assignComboData(talukaaIdCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, talukaaIdCombo);
               },
               failure: function(response, scope) {
                    debugger;
                    scope.sender.controller.addCommunicationLog(response, false);
     ...
     onStateIdChangeTalukaaId: function(me, newValue, oldValue, eOpts) {
          debugger;
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/Taluka/findByStateId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
     ...
     onRegionIdChangeTalukaaId: function(me, newValue, oldValue, eOpts) {
          debugger;
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/Taluka/findByRegionId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
     ...
     onDistrictIdChangeTalukaaId: function(me, newValue, oldValue, eOpts) {
          debugger;
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/Taluka/findByDistrictId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#VillageTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Village');
          var grid = this.view.down('#VillageGrid');
          var tree = this.view.down('#VillageTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#VillageGrid');
               ...
               var formPanel = this.getView().up().down("#Village");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#VillageTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Village').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#VillageGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.villageId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Village').getViewModel().setData(responseData.response.data);
     },
