Ext.define('Buzzor.view.reportui.ReportViewController',{
	...
	renderReport : function(panel, eOpts) {
		...
		if(panel.refId==undefined||panel.refId==null||panel.refId==""){
			Ext.Msg.alert({title:'Error',msg:'Report could not be rendered.Please contact Admin.',icon:Ext.MessageBox.ERROR});
			return;
		}
...
//			  maindatapointpanel.add(newDataPointPanel);
		...
		.request({
			...
			success : function(response, currentObject,
				...
				if (data != undefined
					...
					.loadDataPoints(data.datapoints,currentObject.scope.view.query("mainDataPointPanel")[0]);
				}
			},
			failure : function() {
				Ext.Msg.alert('Error',
	...
	loadGridData : function(queryCriteria, datagrid, scope) {
		...
		var gridStore = this.getGridStore(
				scope.reportJSON.sqlId,sorter, queryCriteria);
